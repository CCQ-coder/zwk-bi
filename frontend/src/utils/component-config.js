...
export const STYLE_DECORATION_STYLE_PRESETS = {
    decor_shape_rect: {
...
export const DEFAULT_COMPONENT_INTERACTION = {
    clickAction: 'filter',
...
export const getChartTypeMeta = (type) => CHART_TYPE_META[type] ?? {
...
export const isStyleDecorationChartType = (type) => STYLE_DECORATION_CHART_TYPES.has(type);
...
const normalizeTableColumnAlign = (value) => (value === 'left' || value === 'right' || value === 'center' ? value : 'center');
const createTableColumnConfig = (field, index, patch) => ({
...
const normalizeTableCustomColumns = (value, fallbackFields) => {
    if (Array.isArray(value)) {
        const normalized = value
            .map((item, index) => {
            if (!item || typeof item !== 'object')
                return null;
            const columnRecord = item;
            const rawField = typeof columnRecord.field === 'string'
                ? columnRecord.field.trim()
                : '';
            if (!rawField)
                return null;
            return createTableColumnConfig(rawField, index, item);
        })
...
export const getConfiguredTableRows = (chartConfig, rawRows, carouselStep = 0) => {
    ...
    if (limitedRows.length <= visibleRows)
        return limitedRows;
...
export const getConfiguredTableStepCount = (chartConfig, rawRows) => {
    const loadLimit = clampInteger(chartConfig.tableLoadLimit, 100, 1, 5000);
    const visibleRows = clampInteger(chartConfig.tableVisibleRows, 10, 1, 200);
    const limitedLength = rawRows.slice(0, loadLimit).length;
    if (limitedLength <= visibleRows)
        return 1;
    const stepSize = chartConfig.tableCarouselMode === 'page' ? visibleRows : 1;
...
const pickColumn = (columns, keywords, excluded = []) => {
...
export const suggestChartFields = (columns, chartType) => {
    if (!columns.length)
        return {};
...
export const parseComponentConfig = (configJson) => {
    ...
    if (typeof parsed.xField === 'string')
        chartPatch.xField = parsed.xField;
    if (typeof parsed.yField === 'string')
    ...
    if (typeof parsed.bgColor === 'string')
        stylePatch.bgColor = parsed.bgColor;
    if (typeof parsed.showLegend === 'boolean')
        stylePatch.showLegend = parsed.showLegend;
    if (typeof parsed.showLabel === 'boolean')
        stylePatch.showLabel = parsed.showLabel;
    if (typeof parsed.labelSize === 'number')
        stylePatch.labelSize = parsed.labelSize;
    if (typeof parsed.showXName === 'boolean')
        stylePatch.showXName = parsed.showXName;
    if (typeof parsed.showYName === 'boolean')
        stylePatch.showYName = parsed.showYName;
    if (typeof parsed.showGrid === 'boolean')
        stylePatch.showGrid = parsed.showGrid;
    if (typeof parsed.smooth === 'boolean')
        stylePatch.smooth = parsed.smooth;
    if (typeof parsed.areaFill === 'boolean')
        stylePatch.areaFill = parsed.areaFill;
    if (typeof parsed.barRadius === 'number')
        stylePatch.barRadius = parsed.barRadius;
    if (typeof parsed.barMaxWidth === 'number')
        stylePatch.barMaxWidth = parsed.barMaxWidth;
    if (typeof parsed.legendPos === 'string')
        stylePatch.legendPos = parsed.legendPos;
    return {
        chart: chartPatch,
        style: stylePatch,
    };
...
export const normalizeComponentDataFilters = (filters) => {
...
export const buildComponentFilterRecord = (filters) => {
    const record = {};
...
export const mergeComponentRequestFilters = (defaultFilters, runtimeFilters) => ({
    ...buildComponentFilterRecord(defaultFilters),
    ...(runtimeFilters ?? {}),
});
export const normalizeComponentAssetConfig = (configJson, baseChart) => {
    const parsed = parseRawComponentConfig(configJson);
...
export const buildComponentConfig = (baseChart, originalConfigJson, patch) => {
...
export const buildComponentAssetConfig = (baseChart, originalConfigJson, patch, layoutPatch) => {
...
export const materializeChartData = (rawRows, columns, chartConfig) => {
    ...
    if (chartConfig.groupField) {
        ...
        rawRows.forEach((row) => {
            ...
            if (!labels.includes(xValue))
                labels.push(xValue);
            const bucket = grouped.get(groupValue) ?? new Map();
...
export const normalizeRuntimeChartData = (data, chartConfig) => {
...
export const buildComponentOption = (data, chartConfig, styleInput) => {
    ...
    const withLegendTextColor = (legend) => legend
        ? { ...legend, textStyle: { ...(legend.textStyle ?? {}), color: textColor } }
        : undefined;
    const withAxisTextColor = (axis) => ({
        ...axis,
    ...
    if (data.chartType === 'bar_combo' || data.chartType === 'bar_combo_group' || data.chartType === 'bar_combo_stack') {
        ...
        return {
            ...
            series: [
                ...barSeries.map((item) => ({ name: item.name, type: 'bar', stack: isComboStack ? 'combo' : undefined, barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [style.barRadius, style.barRadius, 0, 0] }, label: style.showLabel ? { show: true, position: 'top', fontSize: style.labelSize, color: textColor } : { show: false }, data: item.data })),
                ...lineSeries.map((item) => ({ name: item.name, type: 'line', yAxisIndex: 1, smooth: style.smooth, symbol: 'circle', symbolSize: 6, label: style.showLabel ? { show: true, position: 'top', fontSize: style.labelSize, color: textColor } : { show: false }, data: item.data })),
            ],
        };
    }
    // ─── 瀑布图 ────────────────────────────────────────────────────────────────
    if (data.chartType === 'bar_waterfall') {
        ...
        const barVals = [];
        let acc = 0;
        rawVals.forEach((v) => { helpers.push(acc < 0 ? acc + Math.min(v, 0) : acc + Math.min(v, 0) + (v < 0 ? 0 : 0)); barVals.push(v); if (v >= 0) {
            helpers[helpers.length - 1] = acc;
        }
        else {
            helpers[helpers.length - 1] = acc + v;
        } ; acc += v; });
        // Simplified waterfall: helper = cumulative base, actual = delta
        const helper2 = [0];
        const bar2 = [rawVals[0]];
        let cum = rawVals[0];
        for (let i = 1; i < rawVals.length; i++) {
            helper2.push(cum >= 0 ? cum : 0);
            bar2.push(rawVals[i]);
            cum += rawVals[i];
        }
        return {
            color: colors,
            backgroundColor: style.bgColor,
            tooltip: { trigger: 'axis', formatter: (p) => `${p[1]?.axisValue}: ${p[1]?.value}` },
            ...
            series: [
                ...
                { type: 'bar', stack: 'wf', barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [style.barRadius, style.barRadius, 0, 0], color: (p) => rawVals[p.dataIndex] >= 0 ? (colors[0] ?? '#5470c6') : (colors[2] ?? '#ee6666') }, label: style.showLabel ? { show: true, position: 'top', fontSize: style.labelSize, color: textColor } : { show: false }, data: bar2 },
            ],
        };
    }
    // ─── 进度条 ────────────────────────────────────────────────────────────────
    if (data.chartType === 'bar_progress') {
        ...
        return {
            ...
            series: [{ type: 'bar', data: data.series[0].data.map((v) => Number(v) || 0), barMaxWidth: 20, itemStyle: { borderRadius: 10, color: colors[0] ?? '#5470c6' }, label: style.showLabel ? { show: true, position: 'insideRight', fontSize: style.labelSize, color: textColor } : { show: false }, showBackground: true, backgroundStyle: { color: 'rgba(200,200,200,0.15)', borderRadius: 10 } }],
        };
    }
    // ─── 对称条形图 ──────────────────────────────────────────────────────────────
    if (data.chartType === 'bar_horizontal_symmetric') {
        ...
        return {
            ...
            xAxis: withAxisTextColor({ type: 'value', splitLine: style.showGrid ? {} : { show: false }, axisLabel: { formatter: (v) => Math.abs(v).toString() } }),
            yAxis: withAxisTextColor({ type: 'category', data: data.labels }),
            series: [
                { name: data.series[0]?.name ?? '系列1', type: 'bar', barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [0, style.barRadius, style.barRadius, 0], color: colors[0] }, label: style.showLabel ? { show: true, position: 'left', fontSize: style.labelSize, formatter: (p) => Math.abs(p.value).toString(), color: textColor } : { show: false }, data: leftData },
                { name: (data.series[1] ?? data.series[0])?.name ?? '系列2', type: 'bar', barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [style.barRadius, 0, 0, style.barRadius], color: colors[1] ?? colors[0] }, label: style.showLabel ? { show: true, position: 'right', fontSize: style.labelSize, color: textColor } : { show: false }, data: rightData },
            ],
        };
    }
    // ─── 百分比条形图（水平） ────────────────────────────────────────────────────
    if (data.chartType === 'bar_horizontal_percent') {
        if (!data.series.length)
            return { color: colors, backgroundColor: style.bgColor };
        const totals = data.labels.map((_, idx) => data.series.reduce((s, ser) => s + (Number(ser.data[idx]) || 0), 0));
        const shouldShowLegend = style.showLegend && data.series.length > 0;
        return {
            color: colors,
            backgroundColor: style.bgColor,
            ...
            yAxis: withAxisTextColor({ type: 'category', data: data.labels }),
            series: data.series.map((item) => ({ name: item.name, type: 'bar', stack: 'total', barMaxWidth: style.barMaxWidth, label: style.showLabel ? { show: true, fontSize: style.labelSize, formatter: (p) => `${((p.value / totals[p.dataIndex]) * 100).toFixed(1)}%`, color: textColor } : { show: false }, data: item.data })),
        };
    }
    ...
    return {
        ...
        series: (() => {
            if (isPercent && data.series.length > 0) {
                const totals = data.labels.map((_, idx) => data.series.reduce((sum, s) => sum + (Number(s.data[idx]) || 0), 0));
...
const withMinimumTop = (target, minTop) => {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const shouldShiftTopLegend = (legend) => Boolean(legend
    ...
    if (style.showTitle && titleText) {
        ...
        if (Array.isArray(option.legend)) {
            option.legend = option.legend.map((legend) => {
                if (!shouldShiftTopLegend(legend))
                    return legend;
                shiftedLegend = true;
        ...
        if (Array.isArray(option.grid)) {
            option.grid = option.grid.map((grid) => withMinimumTop(grid, minGridTop));
        }
        else if (option.grid) {
