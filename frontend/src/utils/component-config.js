...
export const buildComponentOption = (data, chartConfig, styleInput) => {
    ...
    if (data.chartType === 'bar_combo' || data.chartType === 'bar_combo_group' || data.chartType === 'bar_combo_stack') {
        ...
        return {
            ...
            series: [
                ...barSeries.map((item) => ({ name: item.name, type: 'bar', stack: isComboStack ? 'combo' : undefined, barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [style.barRadius, style.barRadius, 0, 0] }, label: style.showLabel ? { show: true, position: 'top', fontSize: style.labelSize } : { show: false }, data: item.data })),
                ...lineSeries.map((item) => ({ name: item.name, type: 'line', yAxisIndex: 1, smooth: style.smooth, symbol: 'circle', symbolSize: 6, label: style.showLabel ? { show: true, position: 'top', fontSize: style.labelSize } : { show: false }, data: item.data })),
            ],
        };
    }
    // ─── 瀑布图 ────────────────────────────────────────────────────────────────
    if (data.chartType === 'bar_waterfall') {
        ...
        return {
            ...
            series: [
                ...
                { type: 'bar', stack: 'wf', barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [style.barRadius, style.barRadius, 0, 0], color: (p) => rawVals[p.dataIndex] >= 0 ? (colors[0] ?? '#5470c6') : (colors[2] ?? '#ee6666') }, label: style.showLabel ? { show: true, position: 'top', fontSize: style.labelSize } : { show: false }, data: bar2 },
            ],
        };
    }
    // ─── 进度条 ────────────────────────────────────────────────────────────────
    if (data.chartType === 'bar_progress') {
        ...
        return {
            ...
            series: [{ type: 'bar', data: data.series[0].data.map((v) => Number(v) || 0), barMaxWidth: 20, itemStyle: { borderRadius: 10, color: colors[0] ?? '#5470c6' }, label: style.showLabel ? { show: true, position: 'insideRight', fontSize: style.labelSize, color: '#fff' } : { show: false }, showBackground: true, backgroundStyle: { color: 'rgba(200,200,200,0.15)', borderRadius: 10 } }],
        };
    }
    // ─── 对称条形图 ──────────────────────────────────────────────────────────────
    if (data.chartType === 'bar_horizontal_symmetric') {
        ...
        return {
            ...
            xAxis: { type: 'value', splitLine: style.showGrid ? {} : { show: false }, axisLabel: { formatter: (v) => Math.abs(v).toString() } },
            yAxis: { type: 'category', data: data.labels },
            series: [
                { name: data.series[0]?.name ?? '系列1', type: 'bar', barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [0, style.barRadius, style.barRadius, 0], color: colors[0] }, label: style.showLabel ? { show: true, position: 'left', fontSize: style.labelSize, formatter: (p) => Math.abs(p.value).toString() } : { show: false }, data: leftData },
                { name: (data.series[1] ?? data.series[0])?.name ?? '系列2', type: 'bar', barMaxWidth: style.barMaxWidth, itemStyle: { borderRadius: [style.barRadius, 0, 0, style.barRadius], color: colors[1] ?? colors[0] }, label: style.showLabel ? { show: true, position: 'right', fontSize: style.labelSize } : { show: false }, data: rightData },
            ],
        };
    }
    // ─── 百分比条形图（水平） ────────────────────────────────────────────────────
    if (data.chartType === 'bar_horizontal_percent') {
        ...
        return {
            ...
            yAxis: { type: 'category', data: data.labels },
            series: data.series.map((item) => ({ name: item.name, type: 'bar', stack: 'total', barMaxWidth: style.barMaxWidth, label: style.showLabel ? { show: true, fontSize: style.labelSize, formatter: (p) => `${((p.value / totals[p.dataIndex]) * 100).toFixed(1)}%` } : { show: false }, data: item.data })),
        };
    }
