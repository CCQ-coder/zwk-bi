...
public class DashboardService {
    ...
    public BiDashboard create(BiDashboard req) {
        if (!StringUtils.hasText(req.getName())) throw new IllegalArgumentException("仪表板名称不能为空");
    ...
    public BiDashboard update(Long id, BiDashboard req) {
        ...
        if (existing == null) throw new IllegalArgumentException("仪表板不存在: " + id);
    ...
    public DashboardSummaryResponse getDefaultDashboard() {
        ...
        // Use component table to get ordered chart IDs
        ...
        List<BiChart> charts = components.stream()
                .map(c -> biChartMapper.findById(c.getChartId()))
                .filter(c -> c != null)
                .toList();
