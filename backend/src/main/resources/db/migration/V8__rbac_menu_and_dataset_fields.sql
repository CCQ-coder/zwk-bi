...
SET @has_legacy_bi_user := (
  SELECT COUNT(1)
  FROM information_schema.tables
  WHERE table_schema = DATABASE()
    AND table_name = 'bi_user'
);

SET @copy_sys_user_sql := IF(
  @has_legacy_bi_user > 0,
  "INSERT INTO sys_user (id, username, password_hash, display_name, role, email, created_at)
   ...
   FROM bi_user b
   WHERE NOT EXISTS (SELECT 1 FROM sys_user s WHERE s.id = b.id)",
  "SELECT 1"
);
PREPARE copy_sys_user_stmt FROM @copy_sys_user_sql;
EXECUTE copy_sys_user_stmt;
DEALLOCATE PREPARE copy_sys_user_stmt;
...
FROM (
  ...
  UNION ALL
  SELECT '仪表板', '/home/dashboard', 'DashboardView', NULL, 'menu', 'dashboard:view', 'DataBoard', 20, 1, NULL
  UNION ALL
  SELECT '数据大屏', '/home/screen', 'DataScreenView', NULL, 'menu', 'screen:view', 'Monitor', 30, 1, NULL
  UNION ALL
  SELECT '数据准备', '/home/prepare', 'DataPrepareView', NULL, 'menu', 'dataset:view', 'Collection', 40, 1, NULL
  UNION ALL
  SELECT '数据建模', '/home/modeling', 'ModelingView', NULL, 'menu', 'model:view', 'Connection', 50, 1, NULL
  UNION ALL
  SELECT '系统管理', '/home/system', 'SystemView', NULL, 'menu', 'system:view', 'Setting', 60, 1, NULL
) seeded_menu
WHERE NOT EXISTS (
